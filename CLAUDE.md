...
락 3종(비관적 `@Lock(PESSIMISTIC_WRITE)`, 낙관적 `@Version` + 백오프, Redisson `RLock`)은 **조건부 UPDATE만으로 부족해지는 지점을 보여주기 위해** 구현하는 것이지 기본 해법이 아니다. 요청 중복(같은 사용자의 재요청)과 데이터 레이스(정원 경쟁)는 **서로 다른 문제**이며 락은 전자를 해결하지 못한다.
