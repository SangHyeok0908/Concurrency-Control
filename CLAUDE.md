...
**분산 락(Redisson `RLock`)은 도입하지 않기로 판단했다(2026-09-04).** 분산 락은 동시성 제어를 **DB 하나로 끝낼 수 없을 때** — 임계 구역이 외부 결제 API·다른 저장소·정확히 한 번 보내야 하는 알림처럼 **DB 밖의 자원까지** 함께 다뤄야 할 때 — 쓰는 도구인데, 이 도메인의 임계 구역은 슬롯 한 행의 UPDATE 하나다. 판단 근거는 [브랜치 전략 ⑥](docs/STEP2-3-BRANCH-STRATEGY.md#skip-distributed-lock)에 있다. **"다중 인스턴스"나 "락이 하나 빠졌다"를 명분으로 Redisson을 도로 추가하지 말 것** — 인스턴스를 늘려도 공유 상태는 단일 DB 한 곳이고, 그 생략 자체가 문서화된 설계 판단이다. 되살리려면 근거 문서의 전제(임계 구역이 DB 밖으로 확장)가 실제로 바뀌었는지 먼저 확인한다.

**멱등성 키는 도입하지 않기로 판단했다(2026-07-17).** 이 도메인은 (지원자, 슬롯) 자연 키가 요청의 정체성을 고정하고 거기에 UNIQUE가 걸려 있어, 별도 키가 덮을 문제가 남지 않는다. 판단 근거는 [브랜치 전략 ③](docs/STEP2-3-BRANCH-STRATEGY.md#skip-idempotency-key)에 있다. **"중복 요청 방어가 없다"를 명분으로 멱등성 키나 Redis `SETNX`를 도로 추가하지 말 것** — 그 생략 자체가 문서화된 설계 판단이자 포트폴리오의 산출물이다. 되살리려면 근거 문서에 적힌 전제(결제 동반, 복수 예약 허용)가 실제로 바뀌었는지 먼저 확인한다.
