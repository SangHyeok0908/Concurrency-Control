...
- 기존 문서를 우선 수정한다. 새 문서는 기존 문서에 담을 수 없는 목적이 있을 때만 만든다.
- 완료된 작업 계획·AI용 체크리스트는 필요한 판단을 공개 문서에 옮긴 뒤 제거한다. 상세 작업 과정은 Git 이력으로 확인한다.
- 실험별 원시 결과는 한 사본만 유지하고, 이전 실험은 `docs/benchmark/archive/`에 분리한다. 과거 측정값을 새 결과로 덮어쓰지 않는다.
- 실행 도구는 작성자가 실행 흐름을 설명할 수 있는 크기로 유지한다. 로컬 실험에 필요 없는 범용 기능을 추가하지 않는다.
- 도구를 줄여도 실험 완전성·DB 정합성·지표 계산·기존 결과 보호 검증은 유지한다. 실행 방법은 기존 벤치마크 문서에 모은다.
