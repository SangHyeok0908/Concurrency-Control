...
> 이 문서는 baseline·①·②·④·⑤를 같은 시나리오와 같은 애플리케이션 설정에서 두 경합 지점으로
> 측정한 결과다. 2026-09-24에 `benchmark` 프로필로 다시 잰 60행을 현재 정본으로 삼는다.
...
확정 예약·KO·응답시간·TPS는 5회의 중앙값, 응답시간 괄호는 min–max다. 오버부킹과 중복은 한
번이라도 발생하면 실패이므로 5회의 최댓값을 싣는다.
...
|---|---|---|---|---|---|---|---|---|
| baseline (방어 없음) | 9 | 0 | 0 | **111** | 92 <sub>(74–336)</sub> | 131 | 857.1 | 5 |
| ① UNIQUE | 7 | 0 | 0 | **113** | 85 <sub>(72–90)</sub> | 108 | 1081.1 | 5 |
| ② 조건부 UPDATE | 100 | 0 | 0 | 0 | 136 <sub>(109–233)</sub> | 194 | 582.5 | 5 |
| ④ 비관적 락 | 100 | 0 | 0 | 0 | 136 <sub>(125–161)</sub> | 203 | 560.7 | 5 |
| ⑤ 낙관적 락 (상한 5) | 13 | 0 | 0 | **107** | 262 <sub>(240–296)</sub> | 316 | 364.7 | 5 |
| ⑤ 낙관적 락 (상한 20) | 100 | 0 | 0 | 0 | 520 <sub>(375–575)</sub> | 777 | 137.6 | 5 |

### 극단 경합 `cap=1 cont=200`

| 전략 | 확정 예약 | 오버부킹 | 중복 | KO(실패) | 평균 응답 (중앙값) | p95 | TPS | n |
|---|---|---|---|---|---|---|---|---|
| baseline (방어 없음) | 3 | **2** | 0 | **10** | 74 <sub>(61–285)</sub> | 112 | 1652.9 | 5 |
| ① UNIQUE | 3 | **2** | 0 | **10** | 70 <sub>(67–124)</sub> | 106 | 1503.8 | 5 |
| ② 조건부 UPDATE | 1 | 0 | 0 | 0 | 132 <sub>(102–221)</sub> | 194 | 947.9 | 5 |
| ④ 비관적 락 | 1 | 0 | 0 | 0 | 86 <sub>(72–90)</sub> | 120 | 1470.6 | 5 |
| ⑤ 낙관적 락 (상한 5) | 1 | 0 | 0 | 0 | 72 <sub>(70–77)</sub> | 105 | 1639.3 | 5 |
| ⑤ 낙관적 락 (상한 20) | 1 | 0 | 0 | 0 | 74 <sub>(63–98)</sub> | 107 | 1639.3 | 5 |
...
표의 중복 0은 이 부하가 서로 다른 지원자만 쓰기 때문에 나온 값이지, ①을 다시 검증한 수치가
아니다. 같은 지원자의 동시 재요청을 DB가 한 건으로 막는 근거는
[① 전용 동시성 테스트](STEP2-UNIQUE-CONSTRAINT.md)에 있다.
...
| | 오버부킹 | 중복 | 자리를 다 채우는가 | 튜닝 손잡이 | 판단 |
|---|---|---|---|---|---|
| baseline | ❌ 발생 | ❌ | ❌ (대량 KO) | — | 재현 자산 |
| ① UNIQUE | ❌ 발생 | ✅ | ❌ | 없음 | **유지** — 중복 전용 |
| ② 조건부 UPDATE | ✅ 0 | ①에 위임 | ✅ | 없음 | **채택** — 현재 불변식의 가장 작은 표현 |
| ④ 비관적 락 | ✅ 0 | ①에 위임 | ✅ | 없음 | 미채택, 단 성능상 유효 후보 |
| ⑤ 낙관적 락 | ✅ 0 | ①에 위임 | 상한에 따라 | 상한·백오프 | 미채택 — 재시도 비용과 503 |
...
- CSV에 HTTP 상태별 건수와 실행 환경 식별자를 직접 기록해 KO의 원인을 원시 행에서 추적 가능하게
  만든다.
...
검증한다. 결과는 60행이어야 하며 각 `(전략, 상한, 경합)` 조합이 정확히 5행인지 확인한다.
