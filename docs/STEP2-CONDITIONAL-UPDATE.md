...
원자적 연산 하나**로 오버부킹이 사라진다는 것이 이 단계의 논지다. 락(④⑤)은 이 조건부
...
- **DB 밖의 자원 조율.** 단일 DB 안의 오버부킹은 이 한 문장으로 끝나지만, 임계 구역이 DB 밖으로
  나가면(외부 결제 API·다른 저장소·정확히 한 번 알림) 이 문장의 원자성은 거기까지 닿지 못한다 —
  분산 락의 자리다. **이 도메인에는 그 자리가 없어 ⑥은 구현하지 않았다**
  ([근거](STEP2-3-BRANCH-STRATEGY.md#skip-distributed-lock)). 참고로 *다중 인스턴스*는 그 자리를
  만들지 못한다 — 공유 상태가 여전히 단일 DB 한 곳이기 때문이다.
...
| 임계 구역이 **단일 DB 밖**으로 확장<br>(예: 외부 결제 API·다른 서비스 DB·정확히 한 번 알림) | 원자성의 근거인 InnoDB 행 락이 DB 바깥엔 닿지 못함 | 분산 락(Redisson `RLock`)이 자원을 가로지르는 뮤텍스 제공 — **이 도메인엔 이 행이 성립하지 않아 ⑥은 생략**([근거](STEP2-3-BRANCH-STRATEGY.md#skip-distributed-lock)) |

그래서 당시 ⑦ 벤치마크의 예상 결론은 "락이 정합성을 **더** 준다"가 아니었다. 조건부 UPDATE도
락도 오버부킹 0이며, 조건부 UPDATE의 처리량이 더 높을 것이라고 예상했다. 그러나
[2026-09-24 통제 재측정](STEP2-DEFENSE-BENCHMARK.md#conditional-vs-pessimistic)에서는 낮은 경합
응답 중앙값이 ②·④ 모두 136ms였고, 극단 경합은 ④가 빨랐다. 따라서 성능 예측은 기각하고,
현재 선택 근거를 한 행·한 산술 가드를 가장 작은 DML로 표현한다는 점으로 한정한다. 락은
