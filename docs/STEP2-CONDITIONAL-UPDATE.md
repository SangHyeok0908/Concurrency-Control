...
UPDATE만으로 오버부킹이 결정적으로 0이고, 별도 `SELECT ... FOR UPDATE`와 애플리케이션의
read-check-write 구간이 없어 **더 가볍다**([방어 선택 원칙](../PROJECT_PLAN.md#방어-선택-원칙)).
조건부 UPDATE도 트랜잭션이 끝날 때까지 행 잠금을 보유하며 주변 SQL과 결합하면 데드락이 가능하다.
이번 통제 측정의 KO·데드락 0은 해당 시나리오의 관측값이다. 즉 *이 문제 모양에 한해서는 조건부 UPDATE가 사실상 최적해다* —
