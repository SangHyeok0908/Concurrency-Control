...
1단계의 락 없는 baseline은 재현 증거물이다. 방어는 [PROJECT_PLAN의 선택 순서](../PROJECT_PLAN.md#방어-선택-원칙)에 따라
별도 경로로 추가하고 같은 부하에서 비교한다.

### baseline은 보존하고 방어는 additive

`ReservationService`의 락 없는 baseline, `BaselineOverbookingProbeTest`, `V1`, `docs/STEP1-*`는
보존한다. 각 방어는 별도 전략으로 추가하며 baseline 경로에는 [AGENTS.md](../AGENTS.md)의 1단계
불변식이 계속 적용된다.
...
현재 측정의 조건·수치·원시 자료는 [방어 전략 벤치마크](STEP2-DEFENSE-BENCHMARK.md), 최종 선택은
[README 트레이드오프와 최종 선택](../README.md#트레이드오프와-최종-선택)을 정본으로 한다.
과거 고정 순서 측정과 동일 요청 검증은 벤치마크 문서에서 현재 정원 경쟁 결과와 분리한다.
