...
- **중복 *요청*의 응답 의미.** UNIQUE는 기존 결과를 반환하지 않으므로 응답 유실 뒤 재시도는 409를
  받을 수 있다. 자연 키가 충분해 별도 멱등성 키는 생략했고, 남는 개선은 API 응답 계약이다
  ([브랜치 전략 ③](STEP2-3-BRANCH-STRATEGY.md#skip-idempotency-key)).
