 ...
 *
 * <p><b>이 도메인에서 이 전략의 위치.</b> 정합성은 조건부 UPDATE 와 같다. 방법론 v2의 낮은 경합
 * 평균 응답 중앙값은 Phase A/B에서 조건부 125.5/126ms, 이 전략 152/140ms였고, 극단 경합은
 * 조건부 117/122.5ms, 이 전략 88.5/97.5ms였다. Phase 내부 라운드 비교도 같은 방향이지만 두
 * 실행의 직렬 위치와 시점 차이가 남으므로 이를 락 하나의 효과로 단정하지 않는다. 따라서
 * "락이라서 더 느리다"고 일반화하지 않는다. 다만 이 도메인의 불변식은 한 행의
 ...
 * docs/STEP2-DEFENSE-BENCHMARK.md 4절에 있다.
 *
